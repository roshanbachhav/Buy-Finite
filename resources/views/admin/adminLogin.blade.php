...
<body>
    <div class="h-screen text-gray-900 flex justify-center shadow-2xl overflow-hidden">
        <div
            ...
            <div class="lg:w-1/2 xl:w-5/12 p-6 sm:p-12 flex flex-col justify-center">
                ...
                <div class="mt-2 flex flex-col items-center flex-grow">
                    <div class="w-full flex-1">
                        <form action="{{route('postAdminForm')}}" method="POST">
                            @csrf
                            <div class="mx-auto max-w-xs">
                                <input
                                    class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white"
                                    type="email" name="email" placeholder="email@gmail.com" value="{{old('email')}}" />
                                @error('email')
                                <span class="font-medium text-red-400">{{$message}}</span>
                                @enderror
                                <input
                                    class="w-full px-8 py-4 rounded-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white mt-5"
                                    type="password" name="password" placeholder="P@ssword0102"
                                    value="{{old('password')}}" />
                                @error('password')
                                <span class="font-medium text-red-400">{{$message}}</span>
                                @enderror
                                ...
                                <a href="{{route('login')}}" class="flex justify-center text-blue-600"><span
                                        class="ml-2">Go User
                                        Login</span>
                                </a>
                                <p class="mt-6 text-xs text-gray-600 text-center">
                                    This is the admin login page. Unauthorized access is strictly prohibited. Only
                                    authorized personnel are allowed to log in.
                                </p>
                            </div>
                        </form>
                    </div>
