    ...
    <div class="p-4 sm:ml-64 bg-gray-200">
        <div class="container">
            <div class="flex  items-center justify-start text-4xl ml-5 mt-5">
                Edit <span class="text-[#c93eb2] font-semibold mx-2">{{$p->productName}}</span> Details.
            </div>
        ...
        <div class="container mx-auto p-6">
            <form action="{{ route('postEditProduct', $p->id) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
                @csrf

                <div class="container bg-gray-50 border border-gray-400 rounded-2xl p-5 shadow-2xl">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            <input type="text" class="w-full p-2 border border-gray-300 rounded-lg" id="title"
                                name="productName" value="{{old('productName', default: $p->productName)}}" />
                            @error('productName')
                            <p class="text-red-500">{{ $message }}</p>
                            @enderror

                            <label for="message" class="block text-sm font-medium mt-4">Description</label>
                            <textarea rows="4" class="w-full p-2 border border-gray-300 rounded-lg"
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            <input type="text" class="w-full p-2 border border-gray-300 rounded-lg" name="listPrice"
                                id="listPrice" value="{{old('listPrice', $p->listPrice)}}"
                            ...
                            <div class="my-5">
                                ...
                                <input type="text" class="w-full p-2 border border-gray-300 rounded-lg" name="ourPrice"
                                    id="ourPrice" value="{{old('ourPrice',$p->ourPrice)}}"
                                    oninput="calculateDiscount()" />
                                @error('ourPrice')
                                <p class="text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <input type="text" class="w-full p-2 border border-gray-300 rounded-lg" name="off"
                                value="{{old('off',$p->off)}}" id="off" readonly />
                            @error('off')
                            <p class="text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                        <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            <label for="dropzone-file"
                                ...
                                <p class="mb-2 text-sm text-gray-500" id="uploadText">Click to upload or drag and drop
                                </p>
                                <input class="hidden" type="file" id="dropzone-file" name="productImage"
                                    value="{{old('productImage' , $p->productImage)}}" />
                                @error('productImage')
                                <p class="text-red-500">{{ $message }}</p>
                                @enderror
                            </label>
                        ...
                        <div class="bg-white rounded-lg p-6 shadow-2xl">
                            ...
                            <div id="imagePreview"
                                class="w-full h-64 border border-gray-300 rounded-lg flex items-center justify-center overflow-hidden">
                                <img id="previewImg" src="{{ asset('images/product_img/' . $p->productImage) }}"
                                    alt="Image Preview" class="object-cover w-full h-full" />
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                        <div class="bg-white rounded-lg shadow-md p-6">
                            <h5 class="text-lg font-semibold mb-4">Categories & Sub Categories</h5>
                            ...
                            <select name="category_id" id="category_id"
                                ...
                                <option value="{{ $category->id }}" {{ old('category_id', $p->category_id) ==
                            ...
                            <select name="brand_id" id="brand_id"
                                ...
                                <option value="{{ $brand->id }}" {{ old('brand_id', $p->brand_id) ==
                                    $brand->id ? 'selected' : '' }}>
                                    {{ $brand->brand_name }}
                                </option>
                                @endforeach
                        ...
                        <div class="bg-white rounded-lg shadow-md p-6">
                            ...
                            <select name="starRating"
                                class="w-full text-yellow-500 p-2 border border-gray-300 rounded-lg mt-2">
                                <option value="1" class="text-yellow-500" {{ old('starRating', $p->starRating) == 1 ?
                                    'selected' : '' }}>★</option>
                                <option value="2" class="text-yellow-500" {{ old('starRating', $p->starRating) == 2 ?
                                    'selected' : '' }}>★★</option>
                                <option value="3" class="text-yellow-500" {{ old('starRating', $p->starRating) == 3 ?
                                    'selected' : '' }}>★★★</option>
                                <option value="4" class="text-yellow-500" {{ old('starRating', $p->starRating) == 4 ?
                                    'selected' : '' }}>★★★★</option>
                                <option value="5" class="text-yellow-500" {{ old('starRating', $p->starRating) == 5 ?
                                    'selected' : '' }}>★★★★★</option>
                            </select>
                            <label for="linkOfCompany" class="block text-sm font-medium mt-4">Feature or not :</label>
                            <select name="featured" id="featured"
                                class="w-full p-2 border border-gray-300 rounded-lg mt-2">
                                <option value="" disabled {{ old('featured', $p->featured) === null ? 'selected' : ''
                                    }}>Select For Featured Products</option>
                                <option value="0" {{ old('featured', $p->featured) == 0 ? 'selected' : '' }}>NO</option>
                                <option value="1" {{ old('featured', $p->featured) == 1 ? 'selected' : '' }}>YES
                                </option>
                            </select>
                        </div>
                    </div>
